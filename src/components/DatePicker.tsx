import React from 'react';
    ...
    const DatePicker: React.FC<DatePickerProps> = ({ selectedDate, onSelect, onClose }) => {
      ...
      return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-4 w-72" onClick={(e) => e.stopPropagation()}>
            ...
            {days.map((day) => (
              <button
                ...
                className={`
                  ...
                  ${isSelected(day) ? 'bg-green-600 text-white' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}
                  ${isToday(day) ? 'text-green-500 font-bold' : 'text-gray-700 dark:text-gray-300'}
                `}
              >
                {day}
              </button>
